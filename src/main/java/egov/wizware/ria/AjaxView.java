...
public class AjaxView  extends AbstractView
{
    ...
    private final int wizgrid = 5;
    private final int ezmaker = 6;
    private final int webcrea = 7;
    private final int wizdeisgner = 8;
    //--------------------------------------
    ...
    protected void renderMergedOutputModel(Map model, HttpServletRequest request, HttpServletResponse response)  throws Exception
    {
        ...
        else if(rianame == ezmaker)
        {
        	...
        	String totaldata="";
        	StringBuffer data = new StringBuffer(10000);
        	StringBuffer Ezdata = new StringBuffer(10000);
            DOBJ dobj = (DOBJ) model.get("WIZDOBJ");
            String Ezcallback = request.getParameter("callback");
            EzMaker ez = new EzMaker();
         
            Ezdata.append(Ezcallback+"(").toString();
            data = ez.JsonoutputObject(dobj);  
          
            Ezdata.append(data+")").toString();
            //totaldata = callback+"("+data+")";
        //  JSONParser json = new JSONParser();
            // org.json.simple.JSONObject jobj = (org.json.simple.JSONObject)json.parse(data.toString());
           // System.out.println( "-------[Ez Maker  OutData]==START===--->>\n"+jobj);
         //  System.out.println("-------[Ez Maker  OutData]==END===--->>\n" +Ezdata);
           // forwardxJSON(response, jobj);//JSON
           forwardx(response, Ezdata);//data
        }
        else if(rianame == wizgrid)
        {
            StringBuffer data = new StringBuffer(10000);
             ...
             WizGrid jq = new WizGrid();
             // data = jq.outputObject(dobj);  //XML 방식
             data = jq.JsonoutputObject(dobj); //JSON ����

             //JSON �� Layout ���� (multi dataset ���� ó�� : 2015.08.13)
             //data = jq.JsonOutput(dobj);

             JSONParser json = new JSONParser();
             org.json.simple.JSONObject jobj = (org.json.simple.JSONObject)json.parse(data.toString());
           //  System.out.println("-------[wizgrid OutData]---->>"+data);
             // forwardx(response, jobj); //
            // System.out.println(" AJAX VIEW 실행");
             forwardxJSON(response, jobj);//JSON
        }
        ...
        else if(rianame == wizdeisgner)
        {
            StringBuffer data = new StringBuffer(10000);
             DOBJ dobj = (DOBJ) model.get("WIZDOBJ");
             JqGrid jq = new JqGrid();
             // data = jq.outputObject(dobj);  //XML 방식
             data = jq.JsonoutputObject(dobj); //JSON Ңx

             //JSON |ٮ Layout گѦ (multi dataset ڸࠏ óخ : 2015.08.13)
             //data = jq.JsonOutput(dobj);

             JSONParser json = new JSONParser();
             org.json.simple.JSONObject jobj = (org.json.simple.JSONObject)json.parse(data.toString());
             System.out.println(dobj.getRetObjectKeyNames()+"[jqgrid  OutData]---->>"+jobj);
             // forwardx(response, jobj); //
             forwardxJSON(response, jobj);//JSON
        }
        else if(rianame == webcrea)
        {
            StringBuffer data = new StringBuffer(10000);
            DOBJ dobj = (DOBJ) model.get("WIZDOBJ");
            WebCrea mybder = new WebCrea();
            data = mybder.outputObject(dobj);
            System.out.println("-------[WebCrea OutData]---->>"+data);
