...
public class DEVJspWizFrame    extends HttpServlet {
  ...
  public void doPost(HttpServletRequest requestb, HttpServletResponse responseb) throws   ServletException, IOException {
    ...
    if ((requestb.getContentType() != null) &&   (requestb.getContentType().indexOf("multipart") == 0)) {
        ...
        //requestb.setCharacterEncoding("UTF-8");
    ...
    try {
        ...
        System.out.println(jeonmon.get("SYSID") + ":" + jeonmon.get("MENUID").toString() +":" + jeonmon.get("EVENTID").toString());
        ...
        CONNX = connector.getConnection("PFDB");   // 대상(주) 방문판매
        //CONNX = connector.getConnection("FNDB");   // 대상(주) FNF-농협
        //CONNX = connector.getConnectionDirect("oracle","jdbc:oracle:thin:@192.168.16.40:1521:dailydb","FNSH","FNSHX");  // 대상(주) FNF-농협
        ...
        //clog.DevLog(CONNX, dobj.getRetObject("G").getRecord().get("USER_ID"), dobj);
        dobj = this.NonejbExecute(dobj, ctlmap);
        if(dobj.getRtncode() == -1) {
            clog.excepWrite(CONNX, dobj.getRetObject("G").getRecord().get("USER_ID"),  dobj.getRetmsg(), "", dobj);
        }
        tinteval.setEndTime();
        ...
        while(true){
            ...
            if( _tochar[2]=='_' &&
                (  _tochar[1] =='0' || _tochar[1] =='1' ||  _tochar[1]=='2' ||  _tochar[1]=='3' ||  _tochar[1] =='4' ||
                   _tochar[1] =='5' || _tochar[1] =='6' ||  _tochar[1]=='7' ||  _tochar[1]=='8' ||  _tochar[1]=='9'  || ( 65 <= (int)_tochar[1]  && (int)_tochar[1]  <= 90 )
                )
            )
            {
                _cont= false;
