...
public class ControlJspUpWizFrame    extends HttpServlet {
  ...
  public void doPost(HttpServletRequest requestb, HttpServletResponse responseb) throws   ServletException, IOException {
    ...
    try {
        ...
        CONNX = connector.getConnection("FNDB");
        ...
        if(dobj.getRtncode() == -1) {
            clog.excepWrite(CONNX,dobj.getRetObject("G").getRecord().get("I_USERID"), dobj.getRetmsg(), "", dobj);
        }

        tinteval.setEndTime();
        clog.write(CONNX,dobj.getRetObject("G").getRecord().get("I_USERID"), tinteval.getInterval()+"");
